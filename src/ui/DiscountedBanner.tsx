...
const DiscountedBanner = () => {
    ...
    const fetchBrands = async () => {
      try {
        const response = await fetch(
          "https://farm-e-store-backend.vercel.app/api/brand/get-brand"
        );
  ...
  const popularSearchItems = [
    { title: "Insecticides", link: "smartWatches" },
    { title: "Bactericides", link: "headphones" },
    { title: "Herbicides", link: "camerasAndPhotos" },
    { title: "Fungicides", link: "tvAndAudio" },
    { title: "Micro Nutrient", link: "computersAndLaptop" },
  ];
      ...
      <div className="mt-7">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 mt-7">
          {brands.map((brand) => (
            <div
              key={brand._id}
              className="border border-r-0 border-gray-300 flex items-center justify-center px-6 py-2 cursor-pointer group"
            >
              <img
                src={brand.imageUrl}
                alt={brand.title}
